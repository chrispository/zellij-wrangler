...
SESSION="${ZJW_TEST_SESSION:-zjwtest}"
CACHE_DIR="${ZJW_TEST_CACHE:-/tmp/zjwtest-cache}"
...
# would split zjw/hub across directories.
...
ZJW="$CARGO_TARGET_DIR/debug/zjw"
...
check() { # check <desc> <expected-exit> cmd...
    local desc="$1" want="$2"
    shift 2
    "$@" >/tmp/zjwtest-out 2>/tmp/zjwtest-err
    local got=$?
    if [ "$got" -eq "$want" ]; then ok; echo "ok: $desc (exit $got)"; else
        fail "$desc: expected exit $want, got $got"; sed 's/^/    /' /tmp/zjwtest-err | head -3
    fi
}
...
cargo build --manifest-path "$REPO/Cargo.toml" -p zjw || { echo "zjw build failed"; exit 1; }
...
( script -qec "zellij --session $SESSION --config $CONFIG --new-session-with-layout $LAYOUT" /dev/null >/tmp/zjwtest-client.log 2>&1 & )
...
zellij --session "$SESSION" action list-panes --json > /tmp/zjwtest-panes.json || { fail "session did not come up"; exit 1; }
PANE_A=$(jq -r '[.[] | select(.is_plugin == false and .tab_id == 0)][0] | "terminal_\(.id)"' /tmp/zjwtest-panes.json)
PANE_B=$(jq -r '[.[] | select(.is_plugin == false and .tab_id == 0)][1] | "terminal_\(.id)"' /tmp/zjwtest-panes.json)
PANE_C=$(jq -r '[.[] | select(.is_plugin == false and .tab_id == 1)][0] | "terminal_\(.id)"' /tmp/zjwtest-panes.json)
...
export ZJW_HUB_URL="$HUB_URL"

# --- M1 baseline: write-chars + dump-screen + subscribe (cross-tab included) --------------
check "write-chars to $PANE_A" 0 "$ZJW" --session "$SESSION" send "$PANE_A" "marker-A"
sleep 0.5
...
check "write-chars cross-tab to $PANE_C" 0 "$ZJW" --session "$SESSION" send "$PANE_C" "cross-tab-marker"
sleep 0.5
...
# --- target resolution --------------------------------------------------------------------
check "send by tab:1 (active pane of tab 1)" 0 "$ZJW" --session "$SESSION" send "tab:1" "tab-resolved-marker"
check "send by tab-name:work" 0 "$ZJW" --session "$SESSION" send "tab-name:work" "tabname-resolved-marker"
sleep 0.5
...
check "send to missing pane exits 2" 2 "$ZJW" --session "$SESSION" send terminal_999 "nope"
check "send to unknown target exits 2" 2 "$ZJW" --session "$SESSION" send "bogus" "nope"
check "send to unknown tab-name exits 2" 2 "$ZJW" --session "$SESSION" send "tab-name:nope" "nope"
check "send to empty tab exits 2" 2 "$ZJW" --session "$SESSION" send "tab:99" "nope"
check "wait missing pane exits 2" 2 "$ZJW" --session "$SESSION" wait terminal_999 --until x --timeout 500

# --- channels (M3): listen + send --channel ----------------------------------------------
echo "==> starting listeners"
"$ZJW" --session "$SESSION" listen demo > /tmp/zjwtest-listen-raw.out 2>/tmp/zjwtest-listen-raw.err &
LISTEN_RAW=$!
"$ZJW" --session "$SESSION" listen demo --format json > /tmp/zjwtest-listen-json.out 2>/tmp/zjwtest-listen-json.err &
LISTEN_JSON=$!
sleep 2

check "send --channel demo" 0 "$ZJW" --session "$SESSION" send --channel demo "hello listeners"
sleep 1.5
if grep -q "^hello listeners$" /tmp/zjwtest-listen-raw.out; then ok; echo "ok: raw listener received payload"; else fail "raw listener missing payload"; cat /tmp/zjwtest-listen-raw.err; fi
if grep -q '"event":"pipe_output"' /tmp/zjwtest-listen-json.out && grep -q '"output":"hello listeners"' /tmp/zjwtest-listen-json.out; then ok; echo "ok: json listener wrapped payload"; else fail "json listener payload"; cat /tmp/zjwtest-listen-json.out; fi
...
expect_wait_match() { # desc until-pattern pane marker
    ...
    ( sleep 1.5; "$ZJW" --session "$SESSION" send "$pane" "$marker" >/dev/null 2>&1 ) &
    check "$desc" 0 "$ZJW" --session "$SESSION" wait "$pane" --until "$until" --timeout 8000
}
...
check "wait timeout exits 1" 1 "$ZJW" --session "$SESSION" wait "$PANE_A" --until NEVER-MATCHES --timeout 1200
check "wait pre-existing text does not match (exit 1)" 1 "$ZJW" --session "$SESSION" wait "$PANE_A" --until marker-A --timeout 1000

# --- ask (hub-mediated prompt + wait-for-output) ------------------------------------------
check "ask returns new output" 0 "$ZJW" --session "$SESSION" ask "$PANE_A" "echo ASK-REPLY-$RANDOM
" --timeout 8000
if grep -q "ASK-REPLY" /tmp/zjwtest-out; then ok; echo "ok: ask reply contains output"; else fail "ask reply missing output"; cat /tmp/zjwtest-out; fi
check "ask to missing pane exits 2" 2 "$ZJW" --session "$SESSION" ask terminal_999 "x
" --timeout 1000
...
"$ZJW" --session "$SESSION" send "$PANE_B" "stty -echo; sleep 8
" >/dev/null
sleep 0.7
check "ask timeout exits 3" 3 "$ZJW" --session "$SESSION" ask "$PANE_B" "x
" --timeout 1200

# --- status -------------------------------------------------------------------------------
check "status ok" 0 "$ZJW" --session "$SESSION" status "$PANE_A"
if grep -q '"ok": true' /tmp/zjwtest-out; then ok; else fail "status not ok: $(cat /tmp/zjwtest-out)"; fi
check "status unknown pane exits 4" 4 "$ZJW" --session "$SESSION" status terminal_999

# --- targets ------------------------------------------------------------------------------
check "targets lists panes" 0 "$ZJW" --session "$SESSION" targets
check "targets --json parses" 0 "$ZJW" --session "$SESSION" targets --json
